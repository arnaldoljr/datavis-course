function Network() {
  ...
  var countArestas = function(data,id){
    var count = 0;
  ...
  var maxArest = function (data) {
    var max = 0;
    var value = 0;
    data.nodes.forEach(function(n) {
        value = countArestas(data, n.id);
        if (value > max){
          max = value;
        }
      });
  ...
  var minArest = function (data) {
    var min = maxArest(data);
    var value = 0;
    data.nodes.forEach(function(n) {
        value = countArestas(data, n.id);
        if (value < min){
  ...
  function mapArestas(data){
    var arestas;
    arestas = d3.map();
    data.nodes.forEach(function(n){
      return arestas.set(n.id,countArestas(data,n.id));
    });
    return arestas
    }
  ...
  function setupData(data) {
    ...
    data.nodes.forEach(function(n) {
      ...
      //para criar a escala linear eu preciso do piso e do teto do número de arestas
      // add radius to the node so we can use it later
      n.radius = countArestas(data,n.id);
      n.escala = d3.scale.linear().domain([minArest(data),maxArest(data)])
          .range([5,15]);
    ...
    data.links.forEach(function(l) {
      ...
      //l.values = +l.values //seria somente isso?
    });
  ...
  function showDetails(d, i) {
    ...
    content += '<p class="main">' +d.radius + '</span></p>';
